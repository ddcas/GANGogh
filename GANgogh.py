...
from misc.image_params import IMAGE_DIM, IMAGE_OUTPUT_PATH
...
ITERS = 20  # How many iterations to train for
...
PREITERATIONS = 20  # Number of preiteration training cycles to run
...
def kACGANGenerator(n_samples, numClasses, labels, noise=None, dim=IMAGE_DIM, bn=True, nonlinearity=tf.nn.relu,
    ...
    output = lib.ops.linear.Linear('Generator.Input', 128 + numClasses, 8 * 4 * 4 * dim * 2,
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    def generate_image(iteration):
        for i in range(CLASSES):
            ...
            if not os.path.isdir(IMAGE_OUTPUT_PATH):
                os.makedirs(os.path.join(IMAGE_OUTPUT_PATH, 'samples'))
            lib.save_images.save_images(samples.reshape((BATCH_SIZE, 3, IMAGE_DIM, IMAGE_DIM)),
                                        os.path.join(IMAGE_OUTPUT_PATH, 'samples',
                                                     '{}_{}.png'.format(str(i), iteration)))
    ...
    def generate_good_images(iteration, thresh=.95):
        ...
        for i in range(CLASSES):
            ...
            try:
                samples = images
                if not os.path.isdir(IMAGE_OUTPUT_PATH):
                    os.makedirs(os.path.join(IMAGE_OUTPUT_PATH, 'good_samples'))
                lib.save_images.save_images(samples.reshape((-1, 3, IMAGE_DIM, IMAGE_DIM)),
                                            os.path.join(IMAGE_OUTPUT_PATH, 'good_samples', '{}.png'.format(str(i))))
                ...
                for k, image in enumerate(images):
                    if k in indexBase:
                        if isinstance(samples, np.ndarray):
                            ...
                            samples = image
                if not os.path.isdir(IMAGE_OUTPUT_PATH):
                    os.makedirs(os.path.join(IMAGE_OUTPUT_PATH, 'best_samples'))
                lib.save_images.save_images(samples.reshape((-1, 3, IMAGE_DIM, IMAGE_DIM)),
                                            os.path.join(IMAGE_OUTPUT_PATH, 'best_samples', '{}.png'.format(str(i))))
            except Exception as e:
                print(e)
    ...
    if not os.path.isdir(os.path.join(IMAGE_OUTPUT_PATH, 'samples')):
        os.makedirs(os.path.join(IMAGE_OUTPUT_PATH, 'samples'))
    lib.save_images.save_images(_x_r.reshape((BATCH_SIZE, 3, IMAGE_DIM, IMAGE_DIM)),
                                os.path.join(IMAGE_OUTPUT_PATH, 'samples_groundtruth.png'))
    ...
    for iteration in range(ITERS):
        ...
        if (iteration == 9 and iteration < 1000) or iteration % 1000 == 999:
            t = time.time()
            dev_disc_costs = []
        ...
        if (iteration < 2) or (iteration % 100 == 99):
            lib.plot.flush()
